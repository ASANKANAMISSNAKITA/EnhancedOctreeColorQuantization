...
# ----------------- ADDED: DUPLICATE CHECKER PARAM -----------------
DUPLICATE_THRESHOLD = 5  # default RGB distance threshold
...
# ----------------- ADDED: DUPLICATE CHECKER (RGB) -----------------
def check_rgb_near_duplicates(palette, threshold=5):
    """
    Checks all palette pairs using Euclidean RGB distance.
    If distance < threshold, it is reported as near-duplicate.
    """
    print("\n================ DUPLICATE CHECK (ORIGINAL) ================")
    print(f"Threshold (RGB distance) = {threshold}")

    if not palette or len(palette) < 2:
        print("Palette too small to compare.")
        print("============================================================\n")
        return
    ...
    for i in range(Kp):
        for j in range(i + 1, Kp):
            d = math.sqrt(squared_euclidean(palette[i], palette[j]))
            if d < threshold:
                pairs.append((i, j, d))

    if not pairs:
        print("✅ No near-duplicate palette colors found.")
    else:
        pairs.sort(key=lambda x: x[2])
        print(f"⚠ Found {len(pairs)} near-duplicate pair(s):")
        for (i, j, d) in pairs:
            print(f"  Pair ({i},{j}) | {palette[i]} vs {palette[j]} | Distance={d:.2f}")
...
if __name__ == "__main__":
    ...
    # ----------------- ADDED: RUN DUPLICATE CHECKER HERE -----------------
    check_rgb_near_duplicates(final_palette, threshold=DUPLICATE_THRESHOLD)
